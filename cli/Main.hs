...
main = do
  ...
  forConcurrently_ filePaths \filePath -> do
    let
      ...
      encodeString = toS . Turtle.encodeString

      action :: WriterT [Text] IO ()
      action = do
        log $ "processing " <> encodeString filePath
        ...
        log $ "  writing " <> encodeString jsFilePath
        log $ "  writing " <> encodeString pursFilePath

        liftIO $ Turtle.writeTextFile jsFilePath
          ( Text.unlines
            [ "// Do not edit, this file was autogenerated by generate-halogen-css-modules"
            , "exports.styles = require('./" <> toS (encodeString (Turtle.filename filePath)) <> "').default"
            ]
          )
